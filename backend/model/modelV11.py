...
from keras.layers import Conv2D, Flatten, MaxPooling2D, Dense, Dropout, BatchNormalization, ReLU, RandomRotation, RandomContrast, Rescaling, RandomCrop, RandomFlip
import os
...
data_augmentation.add(RandomContrast(0.6))
data_augmentation.add(RandomFlip(
    mode="horizontal_and_vertical"
))

data_augmentation.add(RandomRotation(0.4))
data_augmentation.add(RandomCrop(180, 180))
...
valid_data = tf.keras.preprocessing.image_dataset_from_directory(
    VAL_DATADIR, image_size=(IMG_SIZE, IMG_SIZE), batch_size=32)
...
if __name__ == "__main__":
    ...
    model.save(os.path.join('models', 'BirdClassificationV11.h5'))
